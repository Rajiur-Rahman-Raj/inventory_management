        ...
        <div class="container-fluid">
            ...
            @if(isset($purchaseReportRecords) && count($purchaseReportRecords) > 0 && count($search) > 0)
                <div class="d-flex justify-content-end mb-4">
                    <a href="javascript:void(0)" data-route="{{route('user.export.purchaseReport')}}"
            ...
            @if(isset($purchaseReportRecords) && count($search) > 0)
                <ul class="list-style-none p-0 stock_list_style">
                    <table class="table table-bordered mt-4">
                        ...
                        @if(count($purchaseReportRecords) > 0)
                            @foreach($purchaseReportRecords as $key1 => $purchaseIn)
                                ...
                                @foreach($purchaseIn->rawItemDetails as $key2 => $purchaseInDetails)
                                    <tr>
                                        <td data-label="Quantity">{{ $purchaseIn->supplier->name }}</td>
                                        <td data-label="Quantity">{{ $purchaseInDetails->rawItem->name }}</td>
                                        ...
                                        <td data-label="Total Unit Cost">{{ customDate($purchaseInDetails->purchase_date) }}</td>
                                        <td data-label="Total Unit Cost">{{ $purchaseInDetails->total_unit_cost }} {{ config('basic.currency_symbol') }}</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        @else
                        ...
                        @if(count($purchaseReportRecords) > 0)
                            <tr>
                                <td colspan="5" class="text-right">@lang('Total Price')</td>
